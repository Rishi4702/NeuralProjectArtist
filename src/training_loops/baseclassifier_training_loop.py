...
model_path = r"C:\Users\golur\PycharmProjects\NeuralProjectArtist\runs\Artist\base.pth"
...
model = modify_resnet_model(model, number_of_art)
...
for epoch in range(EPOCHS):
    ...
    for i, data in enumerate(tqdm(training_loader, desc=f"Epoch {epoch + 1}/Training")):
        images, _,artist = data
        images = images.to(device)
        artist = artist.to(device).long()

        optimizer.zero_grad()
        outputs = model(images)
        loss = loss_fn(outputs, artist)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for vdata in tqdm(validation_loader, desc=f"Epoch {epoch + 1}/Validation"):
            vimages,_ ,vartist = vdata
            vimages = vimages.to(device)
            vartist = vartist.to(device).long()

            voutputs = model(vimages)
            vloss = loss_fn(voutputs, vartist)
            total_vloss += vloss.item()

            _, predicted_indices = torch.max(voutputs, 1)
            correct_predictions += (predicted_indices == vartist).sum().item()
            total_predictions += vartist.size(0)
